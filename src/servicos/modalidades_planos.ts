...
export class ModalidadePlanos implements InterfaceCrud<ModalidadesPlanosModel> {
  ...
  async create(
    ...
    const { modalidade_id, plano_id } = payload;
    const query = `
    INSERT INTO modalidades_planos ( modalidade_id,plano_id) 
    values ($1, $2 ) Returning *;
  `;
    const values = [modalidade_id, plano_id];
  ...
  async update(
    ...
    const { modalidade_id, plano_id } = payload;
    const values = [modalidade_id, plano_id, id];
    const result = await this.db.query(
      "UPDATE modalidades_planos SET  modalidade_id  = $1,plano_id= $2 WHERE id = $3 Returning *;",
      values
    );
  ...
  async delete(id: string): Promise<void> {
    const result = await this.db.query("DELETE FROM modalidades_planos WHERE id = $1", [
      id,
    ]);
